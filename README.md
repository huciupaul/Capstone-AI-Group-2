...
This project aims to identify precursor flow states to extreme events using the Kolmogorov flow as a proof of concept. The goal is to provide a novel approach to predicting extreme events in turbulent flows, enabling more efficient and accurate analysis, which can improve the safety, performance, and reliability of systems affected by turbulence in various engineering and scientific applications. The codebase consists of three main components. Namely, 
...
After the generation the data is saved as a H5 file for further analysis. Besides, the disipation rate and kinetic evergy plots are generated for visualization. **This fulflis requirement M1.**
...
* Visualization: Loss plot values are saved every `N_plot` epochs to visualize training and validation trends. A training curve are plotted and saved after early stopping.

For hyperparameter tuning, the `hyperparameter_tuning.py` script trains models over a set of latent space, `n_lat` dimensions and saves the results to `hyperparameter_tuning.txt`. The optimal `n_lat` was selected through a manual trade-off between NRMSE and its size, based on a plot of NRMSE vs latent size.
...
These clustering methods are applied to a normalized and reduced subset to reduce computing times. **These alternative clustering methods complete requirement C2**. Each file plots the clusters found, as well as the extreme events in a 2D scatter plot, showing the x_extreme against the x_0 value.
