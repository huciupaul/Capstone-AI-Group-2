...
Since the analysis is done for turbulent flow, the Reynold's number si set to 40, the treshhold for turbulence in the Kolmogorov flow. Before generating the actual data, there is a run of 2000 steps where the flow model stabilizez.

For the actual generation, a timestep of 0.001 is used, and the data is saved every 20 steps, because too close smaples are very similar, hence it would have been a lot of data but with approximately the same information. During the generation loop, the flow field is generated by the library, whereas the dissipation is computed using the funtion `dissip` and the kinetic energy is calculated manually for every flow field generated step.
